...
function loadWriteTAG_RENAL() {
    ...
    var span = document.createElement("SPAN")
    span.innerHTML =
        `<img class="innerimg TAG" alt="writeTAG_RENAL" id="writeTAG_RENAL" src="../image/icon/lite/tag_RENAL.png" width="50" height="50">`;
    ...
    var span = document.createElement("SPAN")
    span.innerHTML =
        `<img class="img TAG" alt="saveTAG_RENAL" id="saveTAG_RENAL" onmouseover="onElementOver(this);" onmouseleave="onElementLeave();" src="../image/icon/lite/tag_RENAL.png" width="50" height="50" style="display:none;" >`;
    addIconSpan(span);

    var span = document.createElement("SPAN")
    span.innerHTML =
        `<div id="TagStyleDiv_RENAL" style="background-color:#30306044;">
...
function readImageTags_RENAL(url) {
    ...
    request.onload = function () {
        ...
        response.forEach(medicalSpecialityObject => {
            ...
            let selectField = document.getElementById("medicalSpecialtyTag_RENAL");
            ...
            if (medicalSpecialtyName === "R.E.N.A.L." && typeof value['diseases'] === 'object' && !Array.isArray(value['diseases'])) {
                ...
                // 標準數組結構處理
                let diseases = Array.isArray(value['diseases']) ? value['diseases'] : Object.entries(value['diseases']);
                diseases.forEach(diseaseObject => {
                    // 處理標準數組結構
                    let diseaseValue;
                    ...
                    let select = document.createElement('select');
                    select.id = "diseaseSelectorTag_RENAL" + diseaseName.replace(/ /g, "");

                    let selectDiseaseTagNumber = document.querySelectorAll('[id^=diseaseSelectorTag_RENAL]').length
...
getByid("medicalSpecialtyTag_RENAL").onchange = function () {
    let disabledDiseaseDiv = document.getElementById("TagStyleDiv_RENAL").querySelectorAll("div");
...
getByid("saveTAG_RENAL").onclick = function () {
    ...
    if (ConfigLog.Xml2Dcm.enableXml2Dcm == true)
        download2_RENAL(String(get_TAG_context_RENAL()), "" + CreateRandom(), 'text/plain');
    else
        download_RENAL(String(get_TAG_context_RENAL()), sopUID + ".xml", 'text/plain');
...
function set_TAG_context_RENAL(index) {
    ...
    let selectedTag;
    let disabledDiseaseDivs = document.getElementById("TagStyleDiv_RENAL").querySelectorAll("div");

    disabledDiseaseDivs.forEach((elem) => {
        if (elem.hidden === false) {
            //TODO Currently supported just only one disease
            let selector = elem.querySelectorAll("[id^=diseaseSelectorTag_RENAL]")[0];
